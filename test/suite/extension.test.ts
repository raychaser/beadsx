...
suite('Extension Test Suite', () => {
  ...
  test('Commands should be registered', async () => {
    ...
    assert.ok(
      commands.includes('beadsx.refresh'),
      'beadsx.refresh command not registered'
    );
    assert.ok(
      commands.includes('beadsx.filter'),
      'beadsx.filter command not registered'
    );
    assert.ok(
      commands.includes('beadsx.showDetail'),
      'beadsx.showDetail command not registered'
    );
  });
  ...
  test('Configuration should have default values', () => {
    ...
    assert.strictEqual(
      config.get('shortIds'),
      false,
      'Default shortIds should be false'
    );
    assert.strictEqual(
      config.get('autoExpandOpen'),
      true,
      'Default autoExpandOpen should be true'
    );
  });
